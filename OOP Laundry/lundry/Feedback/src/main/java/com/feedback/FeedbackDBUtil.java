...
public class FeedbackDBUtil {
	...
	public static  List<Feedback> validate(String id, String username) {
		...
		try {
			...
			String sql = "select * from feedback where id='"+id+"' and username='"+username+"'";
			...
			if (rs.next()) {
				...
				Feedback c = new Feedback(Id,username1,name,feedback);
				cus.add(c);
			}
    ...
    public static boolean insertcustomer(String username, String name, String feedback) {
    	...
    	try {
    		con = DBConnect.getConnection();
    		stmt = con.createStatement();
    	    String sql = "insert into laundry.feedback values (0,'"+username+"','"+name+"','"+feedback+"')";
